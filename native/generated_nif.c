...
static ERL_NIF_TERM nif_fp_concat_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input_a; if (!enif_inspect_binary(env, argv[0], &bin_input_a)) return enif_make_badarg(env); int64_t* ptr_input_a = (int64_t*)alloc_aligned(bin_input_a.size); memcpy(ptr_input_a, bin_input_a.data, bin_input_a.size);
    ErlNifBinary bin_input_b; if (!enif_inspect_binary(env, argv[1], &bin_input_b)) return enif_make_badarg(env); int64_t* ptr_input_b = (int64_t*)alloc_aligned(bin_input_b.size); memcpy(ptr_input_b, bin_input_b.data, bin_input_b.size);
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[2], &size_output)) return enif_make_badarg(env); int64_t* ptr_output = (int64_t*)alloc_aligned((size_t)size_output);
    ...
    free_aligned(ptr_input_a);
    free_aligned(ptr_input_b);
    ErlNifBinary out_bin_output; enif_alloc_binary((size_t)size_output, &out_bin_output); memcpy(out_bin_output.data, ptr_output, (size_t)size_output); free_aligned(ptr_output);
...
static ERL_NIF_TERM nif_fp_contains_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input; if (!enif_inspect_binary(env, argv[0], &bin_input)) return enif_make_badarg(env); int64_t* ptr_input = (int64_t*)alloc_aligned(bin_input.size); memcpy(ptr_input, bin_input.data, bin_input.size);
...
static ERL_NIF_TERM nif_fp_correlation_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); double* ptr_x = (double*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); double* ptr_y = (double*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
...
static ERL_NIF_TERM nif_fp_detect_outliers_iqr_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_data; if (!enif_inspect_binary(env, argv[0], &bin_data)) return enif_make_badarg(env); double* ptr_data = (double*)alloc_aligned(bin_data.size); memcpy(ptr_data, bin_data.data, bin_data.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    double val_k_factor; if (!enif_get_double(env, argv[2], &val_k_factor)) return enif_make_badarg(env);
    ErlNifUInt64 size_is_outlier; if (!enif_get_uint64(env, argv[3], &size_is_outlier)) return enif_make_badarg(env); uint8_t* ptr_is_outlier = (uint8_t*)alloc_aligned((size_t)size_is_outlier);
    size_t res = fp_detect_outliers_iqr_f64(ptr_data, val_n, val_k_factor, ptr_is_outlier);
    free_aligned(ptr_data);
    ErlNifBinary out_bin_is_outlier; enif_alloc_binary((size_t)size_is_outlier, &out_bin_is_outlier); memcpy(out_bin_is_outlier.data, ptr_is_outlier, (size_t)size_is_outlier); free_aligned(ptr_is_outlier);
...
static ERL_NIF_TERM nif_fp_drop_n_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input; if (!enif_inspect_binary(env, argv[0], &bin_input)) return enif_make_badarg(env); int64_t* ptr_input = (int64_t*)alloc_aligned(bin_input.size); memcpy(ptr_input, bin_input.data, bin_input.size);
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[1], &size_output)) return enif_make_badarg(env); int64_t* ptr_output = (int64_t*)alloc_aligned((size_t)size_output);
...
static ERL_NIF_TERM nif_fp_ema_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[3], &size_output)) return enif_make_badarg(env); double* ptr_output = (double*)alloc_aligned((size_t)size_output);
...
static ERL_NIF_TERM nif_fp_fold_dotp_f32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); float* ptr_a = (float*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); float* ptr_b = (float*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); double* ptr_a = (double*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); double* ptr_b = (double*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_i16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); int16_t* ptr_a = (int16_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); int16_t* ptr_b = (int16_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_i32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); int32_t* ptr_a = (int32_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); int32_t* ptr_b = (int32_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); int64_t* ptr_a = (int64_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); int64_t* ptr_b = (int64_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_i8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); int8_t* ptr_a = (int8_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); int8_t* ptr_b = (int8_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_u16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); uint16_t* ptr_a = (uint16_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); uint16_t* ptr_b = (uint16_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_u32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); uint32_t* ptr_a = (uint32_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); uint32_t* ptr_b = (uint32_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_u64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); uint64_t* ptr_a = (uint64_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); uint64_t* ptr_b = (uint64_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_dotp_u8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_a; if (!enif_inspect_binary(env, argv[0], &bin_a)) return enif_make_badarg(env); uint8_t* ptr_a = (uint8_t*)alloc_aligned(bin_a.size); memcpy(ptr_a, bin_a.data, bin_a.size);
    ErlNifBinary bin_b; if (!enif_inspect_binary(env, argv[1], &bin_b)) return enif_make_badarg(env); uint8_t* ptr_b = (uint8_t*)alloc_aligned(bin_b.size); memcpy(ptr_b, bin_b.data, bin_b.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_f32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); float* ptr_in = (float*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_i16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int16_t* ptr_in = (int16_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_i32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int32_t* ptr_in = (int32_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int64_t* ptr_in = (int64_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_i8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int8_t* ptr_in = (int8_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_u16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint16_t* ptr_in = (uint16_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_u32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint32_t* ptr_in = (uint32_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_u64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint64_t* ptr_in = (uint64_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_fold_sumsq_u8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint8_t* ptr_in = (uint8_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_gaussian_nb_predict_batch(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    ErlNifBinary bin_X; if (!enif_inspect_binary(env, argv[1], &bin_X)) return enif_make_badarg(env); double* ptr_X = (double*)alloc_aligned(bin_X.size); memcpy(ptr_X, bin_X.data, bin_X.size);
    int val_n; if (!enif_get_int(env, argv[2], (int*)&val_n)) return enif_make_badarg(env);
    ErlNifUInt64 size_predictions; if (!enif_get_uint64(env, argv[3], &size_predictions)) return enif_make_badarg(env); int* ptr_predictions = (int*)alloc_aligned((size_t)size_predictions);
    fp_gaussian_nb_predict_batch(res_model, ptr_X, val_n, ptr_predictions);
    free_aligned(ptr_X);
    ErlNifBinary out_bin_predictions; enif_alloc_binary((size_t)size_predictions, &out_bin_predictions); memcpy(out_bin_predictions.data, ptr_predictions, (size_t)size_predictions); free_aligned(ptr_predictions);
...
static ERL_NIF_TERM nif_fp_gaussian_nb_train(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_X; if (!enif_inspect_binary(env, argv[0], &bin_X)) return enif_make_badarg(env); double* ptr_X = (double*)alloc_aligned(bin_X.size); memcpy(ptr_X, bin_X.data, bin_X.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); int* ptr_y = (int*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    int val_n; if (!enif_get_int(env, argv[2], (int*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_group_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input; if (!enif_inspect_binary(env, argv[0], &bin_input)) return enif_make_badarg(env); int64_t* ptr_input = (int64_t*)alloc_aligned(bin_input.size); memcpy(ptr_input, bin_input.data, bin_input.size);
    ErlNifUInt64 size_groups_out; if (!enif_get_uint64(env, argv[1], &size_groups_out)) return enif_make_badarg(env); int64_t* ptr_groups_out = (int64_t*)alloc_aligned((size_t)size_groups_out);
    ErlNifUInt64 size_counts_out; if (!enif_get_uint64(env, argv[2], &size_counts_out)) return enif_make_badarg(env); int64_t* ptr_counts_out = (int64_t*)alloc_aligned((size_t)size_counts_out);
    ...
    free_aligned(ptr_input);
    ErlNifBinary out_bin_groups_out; enif_alloc_binary((size_t)size_groups_out, &out_bin_groups_out); memcpy(out_bin_groups_out.data, ptr_groups_out, (size_t)size_groups_out); free_aligned(ptr_groups_out);
    ErlNifBinary out_bin_counts_out; enif_alloc_binary((size_t)size_counts_out, &out_bin_counts_out); memcpy(out_bin_counts_out.data, ptr_counts_out, (size_t)size_counts_out); free_aligned(ptr_counts_out);
...
static ERL_NIF_TERM nif_fp_intersect_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_array_a; if (!enif_inspect_binary(env, argv[0], &bin_array_a)) return enif_make_badarg(env); int64_t* ptr_array_a = (int64_t*)alloc_aligned(bin_array_a.size); memcpy(ptr_array_a, bin_array_a.data, bin_array_a.size);
    ErlNifBinary bin_array_b; if (!enif_inspect_binary(env, argv[1], &bin_array_b)) return enif_make_badarg(env); int64_t* ptr_array_b = (int64_t*)alloc_aligned(bin_array_b.size); memcpy(ptr_array_b, bin_array_b.data, bin_array_b.size);
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[2], &size_output)) return enif_make_badarg(env); int64_t* ptr_output = (int64_t*)alloc_aligned((size_t)size_output);
    size_t val_len_a; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_len_a)) return enif_make_badarg(env);
    size_t val_len_b; if (!enif_get_uint64(env, argv[4], (ErlNifUInt64*)&val_len_b)) return enif_make_badarg(env);
    size_t res = fp_intersect_i64(ptr_array_a, ptr_array_b, ptr_output, val_len_a, val_len_b);
    free_aligned(ptr_array_a);
    free_aligned(ptr_array_b);
    ErlNifBinary out_bin_output; enif_alloc_binary((size_t)size_output, &out_bin_output); memcpy(out_bin_output.data, ptr_output, (size_t)size_output); free_aligned(ptr_output);
    return enif_make_tuple2(env, enif_make_uint64(env, res), enif_make_binary(env, &out_bin_output));
}

static ERL_NIF_TERM nif_fp_iterate_add_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[0], &size_output)) return enif_make_badarg(env); int64_t* ptr_output = (int64_t*)alloc_aligned((size_t)size_output);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_linear_regression_r2_score(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_y_true; if (!enif_inspect_binary(env, argv[0], &bin_y_true)) return enif_make_badarg(env); double* ptr_y_true = (double*)alloc_aligned(bin_y_true.size); memcpy(ptr_y_true, bin_y_true.data, bin_y_true.size);
    ErlNifBinary bin_y_pred; if (!enif_inspect_binary(env, argv[1], &bin_y_pred)) return enif_make_badarg(env); double* ptr_y_pred = (double*)alloc_aligned(bin_y_pred.size); memcpy(ptr_y_pred, bin_y_pred.data, bin_y_pred.size);
    int val_n; if (!enif_get_int(env, argv[2], (int*)&val_n)) return enif_make_badarg(env);
    double res = fp_linear_regression_r2_score(ptr_y_true, ptr_y_pred, val_n);
    free_aligned(ptr_y_true);
    free_aligned(ptr_y_pred);
    
    return enif_make_double(env, res);
}

static ERL_NIF_TERM nif_fp_map_abs_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); double* ptr_in = (double*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); double* ptr_out = (double*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    fp_map_abs_f64(ptr_in, ptr_out, val_n);
    free_aligned(ptr_in);
    ErlNifBinary out_bin_out; enif_alloc_binary((size_t)size_out, &out_bin_out); memcpy(out_bin_out.data, ptr_out, (size_t)size_out); free_aligned(ptr_out);
...
static ERL_NIF_TERM nif_fp_map_abs_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int64_t* ptr_in = (int64_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); int64_t* ptr_out = (int64_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_f32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); float* ptr_x = (float*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); float* ptr_y = (float*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); float* ptr_out = (float*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); double* ptr_x = (double*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); double* ptr_y = (double*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); double* ptr_out = (double*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_i16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); int16_t* ptr_x = (int16_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); int16_t* ptr_y = (int16_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); int16_t* ptr_out = (int16_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_i32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); int32_t* ptr_x = (int32_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); int32_t* ptr_y = (int32_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); int32_t* ptr_out = (int32_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); int64_t* ptr_x = (int64_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); int64_t* ptr_y = (int64_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); int64_t* ptr_out = (int64_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_i8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); int8_t* ptr_x = (int8_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); int8_t* ptr_y = (int8_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); int8_t* ptr_out = (int8_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_u16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); uint16_t* ptr_x = (uint16_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); uint16_t* ptr_y = (uint16_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); uint16_t* ptr_out = (uint16_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_u32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); uint32_t* ptr_x = (uint32_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); uint32_t* ptr_y = (uint32_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); uint32_t* ptr_out = (uint32_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_u64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); uint64_t* ptr_x = (uint64_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); uint64_t* ptr_y = (uint64_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); uint64_t* ptr_out = (uint64_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_axpy_u8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); uint8_t* ptr_x = (uint8_t*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_y; if (!enif_inspect_binary(env, argv[1], &bin_y)) return enif_make_badarg(env); uint8_t* ptr_y = (uint8_t*)alloc_aligned(bin_y.size); memcpy(ptr_y, bin_y.data, bin_y.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[2], &size_out)) return enif_make_badarg(env); uint8_t* ptr_out = (uint8_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_f32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); float* ptr_in = (float*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); float* ptr_out = (float*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_i16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int16_t* ptr_in = (int16_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); int16_t* ptr_out = (int16_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_i32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int32_t* ptr_in = (int32_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); int32_t* ptr_out = (int32_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_i8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); int8_t* ptr_in = (int8_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); int8_t* ptr_out = (int8_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_u16(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint16_t* ptr_in = (uint16_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); uint16_t* ptr_out = (uint16_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_u32(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint32_t* ptr_in = (uint32_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); uint32_t* ptr_out = (uint32_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_u64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint64_t* ptr_in = (uint64_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); uint64_t* ptr_out = (uint64_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_map_offset_u8(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_in; if (!enif_inspect_binary(env, argv[0], &bin_in)) return enif_make_badarg(env); uint8_t* ptr_in = (uint8_t*)alloc_aligned(bin_in.size); memcpy(ptr_in, bin_in.data, bin_in.size);
    ErlNifUInt64 size_out; if (!enif_get_uint64(env, argv[1], &size_out)) return enif_make_badarg(env); uint8_t* ptr_out = (uint8_t*)alloc_aligned((size_t)size_out);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_moments_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_data; if (!enif_inspect_binary(env, argv[0], &bin_data)) return enif_make_badarg(env); double* ptr_data = (double*)alloc_aligned(bin_data.size); memcpy(ptr_data, bin_data.data, bin_data.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    ErlNifUInt64 size_moments; if (!enif_get_uint64(env, argv[2], &size_moments)) return enif_make_badarg(env); double* ptr_moments = (double*)alloc_aligned((size_t)size_moments);
    fp_moments_f64(ptr_data, val_n, ptr_moments);
    free_aligned(ptr_data);
    ErlNifBinary out_bin_moments; enif_alloc_binary((size_t)size_moments, &out_bin_moments); memcpy(out_bin_moments.data, ptr_moments, (size_t)size_moments); free_aligned(ptr_moments);
...
static ERL_NIF_TERM nif_fp_neural_network_train(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int val_n_hidden; if (!enif_get_int(env, argv[1], (int*)&val_n_hidden)) return enif_make_badarg(env);
    int val_n_outputs; if (!enif_get_int(env, argv[2], (int*)&val_n_outputs)) return enif_make_badarg(env);
    ErlNifBinary bin_X_train; if (!enif_inspect_binary(env, argv[3], &bin_X_train)) return enif_make_badarg(env); double* ptr_X_train = (double*)alloc_aligned(bin_X_train.size); memcpy(ptr_X_train, bin_X_train.data, bin_X_train.size);
    ErlNifBinary bin_y_train; if (!enif_inspect_binary(env, argv[4], &bin_y_train)) return enif_make_badarg(env); double* ptr_y_train = (double*)alloc_aligned(bin_y_train.size); memcpy(ptr_y_train, bin_y_train.data, bin_y_train.size);
    ...
    *res_ptr = res;
    ...
    free_aligned(ptr_X_train);
    free_aligned(ptr_y_train);
...
static ERL_NIF_TERM nif_fp_partition_gt_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input; if (!enif_inspect_binary(env, argv[0], &bin_input)) return enif_make_badarg(env); int64_t* ptr_input = (int64_t*)alloc_aligned(bin_input.size); memcpy(ptr_input, bin_input.data, bin_input.size);
    ErlNifUInt64 size_output_pass; if (!enif_get_uint64(env, argv[1], &size_output_pass)) return enif_make_badarg(env); int64_t* ptr_output_pass = (int64_t*)alloc_aligned((size_t)size_output_pass);
    ErlNifUInt64 size_output_fail; if (!enif_get_uint64(env, argv[2], &size_output_fail)) return enif_make_badarg(env); int64_t* ptr_output_fail = (int64_t*)alloc_aligned((size_t)size_output_fail);
    size_t val_n; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    int64_t val_threshold; if (!enif_get_int64(env, argv[4], (ErlNifSInt64*)&val_threshold)) return enif_make_badarg(env);
    ErlNifUInt64 size_out_pass_count; if (!enif_get_uint64(env, argv[5], &size_out_pass_count)) return enif_make_badarg(env); size_t* ptr_out_pass_count = (size_t*)alloc_aligned((size_t)size_out_pass_count);
    ErlNifUInt64 size_out_fail_count; if (!enif_get_uint64(env, argv[6], &size_out_fail_count)) return enif_make_badarg(env); size_t* ptr_out_fail_count = (size_t*)alloc_aligned((size_t)size_out_fail_count);
    fp_partition_gt_i64(ptr_input, ptr_output_pass, ptr_output_fail, val_n, val_threshold, ptr_out_pass_count, ptr_out_fail_count);
    free_aligned(ptr_input);
    ErlNifBinary out_bin_output_pass; enif_alloc_binary((size_t)size_output_pass, &out_bin_output_pass); memcpy(out_bin_output_pass.data, ptr_output_pass, (size_t)size_output_pass); free_aligned(ptr_output_pass);
    ErlNifBinary out_bin_output_fail; enif_alloc_binary((size_t)size_output_fail, &out_bin_output_fail); memcpy(out_bin_output_fail.data, ptr_output_fail, (size_t)size_output_fail); free_aligned(ptr_output_fail);
    ErlNifBinary out_bin_out_pass_count; enif_alloc_binary((size_t)size_out_pass_count, &out_bin_out_pass_count); memcpy(out_bin_out_pass_count.data, ptr_out_pass_count, (size_t)size_out_pass_count); free_aligned(ptr_out_pass_count);
    ErlNifBinary out_bin_out_fail_count; enif_alloc_binary((size_t)size_out_fail_count, &out_bin_out_fail_count); memcpy(out_bin_out_fail_count.data, ptr_out_fail_count, (size_t)size_out_fail_count); free_aligned(ptr_out_fail_count);
...
static ERL_NIF_TERM nif_fp_pca_generate_ellipse_data(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifUInt64 size_X; if (!enif_get_uint64(env, argv[0], &size_X)) return enif_make_badarg(env); double* ptr_X = (double*)alloc_aligned((size_t)size_X);
    int val_n; if (!enif_get_int(env, argv[1], (int*)&val_n)) return enif_make_badarg(env);
    ...
    ErlNifBinary out_bin_X; enif_alloc_binary((size_t)size_X, &out_bin_X); memcpy(out_bin_X.data, ptr_X, (size_t)size_X); free_aligned(ptr_X);
...
static ERL_NIF_TERM nif_fp_percentiles_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_data; if (!enif_inspect_binary(env, argv[0], &bin_data)) return enif_make_badarg(env); double* ptr_data = (double*)alloc_aligned(bin_data.size); memcpy(ptr_data, bin_data.data, bin_data.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    ErlNifBinary bin_p_values; if (!enif_inspect_binary(env, argv[2], &bin_p_values)) return enif_make_badarg(env); double* ptr_p_values = (double*)alloc_aligned(bin_p_values.size); memcpy(ptr_p_values, bin_p_values.data, bin_p_values.size);
    size_t val_n_percentiles; if (!enif_get_uint64(env, argv[3], (ErlNifUInt64*)&val_n_percentiles)) return enif_make_badarg(env);
    ErlNifUInt64 size_results; if (!enif_get_uint64(env, argv[4], &size_results)) return enif_make_badarg(env); double* ptr_results = (double*)alloc_aligned((size_t)size_results);
    fp_percentiles_f64(ptr_data, val_n, ptr_p_values, val_n_percentiles, ptr_results);
    free_aligned(ptr_data);
    free_aligned(ptr_p_values);
    ErlNifBinary out_bin_results; enif_alloc_binary((size_t)size_results, &out_bin_results); memcpy(out_bin_results.data, ptr_results, (size_t)size_results); free_aligned(ptr_results);
...
static ERL_NIF_TERM nif_fp_pred_all_eq_const_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_arr; if (!enif_inspect_binary(env, argv[0], &bin_arr)) return enif_make_badarg(env); int64_t* ptr_arr = (int64_t*)alloc_aligned(bin_arr.size); memcpy(ptr_arr, bin_arr.data, bin_arr.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_reduce_add_f64_where(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_x; if (!enif_inspect_binary(env, argv[0], &bin_x)) return enif_make_badarg(env); double* ptr_x = (double*)alloc_aligned(bin_x.size); memcpy(ptr_x, bin_x.data, bin_x.size);
    ErlNifBinary bin_mask; if (!enif_inspect_binary(env, argv[1], &bin_mask)) return enif_make_badarg(env); int* ptr_mask = (int*)alloc_aligned(bin_mask.size); memcpy(ptr_mask, bin_mask.data, bin_mask.size);
    size_t val_n; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    double res = fp_reduce_add_f64_where(ptr_x, ptr_mask, val_n);
    free_aligned(ptr_x);
    free_aligned(ptr_mask);
...
static ERL_NIF_TERM nif_fp_reduce_product_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_input; if (!enif_inspect_binary(env, argv[0], &bin_input)) return enif_make_badarg(env); double* ptr_input = (double*)alloc_aligned(bin_input.size); memcpy(ptr_input, bin_input.data, bin_input.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_replicate_f64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[0], &size_output)) return enif_make_badarg(env); double* ptr_output = (double*)alloc_aligned((size_t)size_output);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
...
static ERL_NIF_TERM nif_fp_rolling_max_i64(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary bin_data; if (!enif_inspect_binary(env, argv[0], &bin_data)) return enif_make_badarg(env); int64_t* ptr_data = (int64_t*)alloc_aligned(bin_data.size); memcpy(ptr_data, bin_data.data, bin_data.size);
    size_t val_n; if (!enif_get_uint64(env, argv[1], (ErlNifUInt64*)&val_n)) return enif_make_badarg(env);
    size_t val_window; if (!enif_get_uint64(env, argv[2], (ErlNifUInt64*)&val_window)) return enif_make_badarg(env);
    ErlNifUInt64 size_output; if (!enif_get_uint64(env, argv[3], &size_output)) return enif_make_badarg(env); int64_t* ptr_output = (int64_t*)alloc_aligned((size_t)size_output);
